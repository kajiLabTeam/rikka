...
_LOCAL_OFFSET_DEGREES = np.array(
    [0.0, 5.0, -5.0, 10.0, -10.0, 20.0, -20.0, 30.0, -30.0, 45.0, -45.0]
)
_TURN_OFFSET_DEGREES = np.array([60.0, -60.0, 90.0, -90.0, 135.0, -135.0, 180.0])


def _recovery_length_factors(allow_stride_adaptation: bool) -> np.ndarray:
    """歩幅状態の適応有無に応じた共通候補倍率を返す。"""
    if allow_stride_adaptation:
        return np.array([1.0, 0.85, 0.7, 1.15, 1.3])
    return np.array([1.0, 0.9, 1.1])
...
def _generate_recovery_candidates(
    ...
    *,
    ...
    normal_length_factors = _recovery_length_factors(allow_stride_adaptation)
    stages = [("local_grid", _LOCAL_OFFSET_DEGREES, normal_length_factors)]
    if allow_turn_candidates:
        stages.append(
            (
                "turn_grid",
                _TURN_OFFSET_DEGREES,

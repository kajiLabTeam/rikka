...
役割:
    CLI や互換 API からの PDR 実行を統括し、通常軌跡と particle filter の分岐、
    CSV 保存、グラフ生成までをまとめる。
依存元:
    ``common`` から設定・共有部品、各領域の pipeline と plot 部品を取得する。
利用先:
...
from ..common.lib.models import FloorMap, GyroBiasResult
...
def _validate_particle_floormap(
    ...
    from ..particle.lib.map_constraints import (  # noqa: PLC0415
...
def run(
    ...
    if use_particle_filter:
        from ..particle.lib.recorder import (  # noqa: PLC0415
            ParticleFilterStepDiagnostics,
        )
        from ..plot.lib.animation import (  # noqa: PLC0415
        ...
        if save_step_frames or save_path_comparison:
            from ..plot.lib.frames import (  # noqa: PLC0415
        ...
        if plot:
            ...
            from ..plot.lib.sensor import (  # noqa: PLC0415
                plot_step_lengths,
                plot_step_vectors,
            )

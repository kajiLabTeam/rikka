...
| 型定義 | [`ParticleFilterResult`](../../src/rikka/common/lib/models.py#L260)の診断が`tuple[Any,...]` | 境界型が弱い | recorder型を循環なしで共有型化 | models/recorder | 低 | import循環 |
| 関数分割 | [`run_particle_steps`](../../src/rikka/particle/lib/runner.py#L74)は42引数 | 読解・呼出ミス | prepared/config/runtime factoryへ束ねる | particle API/tests | 中 | 外部低水準API互換 |
| 責務整理 | [`cli.commands.run()`](../../src/rikka/cli/commands.py#L81)が実行と保存を常に結合 | 純粋APIでもoutputを生成 | analyze結果返却と保存adapterを分ける | CLI/Python API | 中 | 現公開APIの副作用変更 |
...
| 未使用コード整理 | [`_snap_trajectory_to_walkable_pixels`](../../src/rikka/particle/lib/map_constraints.py#L170)の呼出なし | 用途判断が必要 | 意図確認後、実験用明示化か削除提案 | map_constraints/tests | 低 | 隠れた外部利用 |
| ファイル分割 | [`cli/options.py`](../../src/rikka/cli/options.py)等が400行超 | 目的のoption探索に時間 | コマンド別decoratorへ分割 | CLI | 低 | Click decorator順序 |
...
最初に「現行既定入力に対応する truth・取得条件」を確定し、その後に評価gateを追加するのが安全です。

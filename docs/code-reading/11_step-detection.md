...
選べ、後者だけ明示的な1歩区間 `StepSegment` を返します。
...
|---|---|
| 入力データ | 前処理済み加速度 |
| 入力元 | `process_sensor_data()` |
| 出力データ | `StepDetectionResult` |
...
|---|---|---|---|---|---|
| `detect_step_result` | `pdr/lib/step_detection.py` | 方式分岐 | acc DF、method | 結果型 | preparation |
| `_detect_steps_by_peak` | 同上 | 標準ピーク検出 | `low_lin_norm` | peaks | 上記 |
| `_detect_steps_by_vertical_threshold` | 同上 | 区間抽出 | `v_acc` | peaks/segments | 上記 |
| `_suppress_close_contacts` | 同上 | 近接重複抑制 | index/strength | 接地候補 | paper方式 |
...
- `detect_steps()`: peaksだけを必要とする互換・検証用API。
...
- `src/rikka/pdr/lib/step_detection.py`
- `src/rikka/common/lib/models.py`
